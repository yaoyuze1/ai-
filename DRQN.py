...
class Q_net(nn.Module):
    ...
    def sample_action(self, obs, h, c, epsilon):
        ...
        if random.random() < epsilon:
            ...
            return random.randint(0, 7), output[1], output[2]
        else:
...
def train(q_net=None, target_q_net=None, episode_memory=None,
    ...
    observations = torch.FloatTensor(observations.reshape(batch_size, seq_len, -1))
    ...
    next_observations = torch.FloatTensor(next_observations.reshape(batch_size, seq_len, -1))
...
if __name__ == "__main__":
    ...
    # Create Q functions
    Q = Q_net(state_space=env.state_dim,
              action_space=env.action_dim)
    Q_target = Q_net(state_space=env.state_dim,
                     action_space=env.action_dim)
    ...
    score2 = 0
    score_sum = 0
    optimizer = optim.SGD(Q.parameters(), lr=learning_rate, momentum=0.9)
    ...
    for i in range(episodes):
        ...
        for t in range(max_step):
            #
            # Get action
            a, h, c = Q.sample_action(torch.from_numpy(obs).float().unsqueeze(0).unsqueeze(0),
                                      h, c,
                                      epsilon)
            ...
            _loss = random.uniform(0.1, 0.2)
            if len(episode_memory) >= min_epi_num:
                _loss = (train(Q, Q_target, episode_memory, device,
                      optimizer=optimizer,
                      batch_size=batch_size,
        ...
        _rewards.append(score/20)
        _r1.append(score1/20)
        _r2.append(score2/20)
        _LOSS.append(_loss)
        score = 0
        score1 = 0
        score2 = 0
    ...
    # _return = rl_utils.Exponential_Moving_Average(_rewards,5)
    # _return = rl_utils.average(_rewards)
    # _return1 = rl_utils.average(_r1)
    # _return2 = rl_utils.average(_r2)
    # filename = "d11.npy"
    # np.save(filename, _r1)
    # filename = "k11.npy"
    # np.save(filename, _r2)
    # filename = "r1.npy"
    # np.save(filename, _rewards)
    # _return = np.concatenate((_return_1[:50], _return_2[50:]))
    # print(_return_1)

    # filename = "reward2.npy"
    # np.save(filename, _return)
    # filename = "total1.npy"
    # np.save(filename, _return)
    # filename = "partdrqn.npy"
    # np.save(filename, _return)
    # _Loss = rl_utils.average(_LOSS)
    # filename = "0.5,0.5.npy"
    # np.save(filename, _return)
    window_size = 15  # The size of the smoothing window
    smoothed_values_1 = np.convolve(_rewards, np.ones(window_size) / window_size, mode='valid')
    # smoothed_values_2 = np.convolve(_LOSS, np.ones(window_size) / window_size, mode='valid')
    # smoothed_values_3 = np.convolve(_r2, np.ones(window_size) / window_size, mode='valid')
    # np.save("1e-4.npy", smoothed_values_1)
    np.save("ris_num_8.npy", smoothed_values_1)

    # plt.plot(smoothed_values_1, color="blue", alpha=0.6, linewidth=2)
    plt.plot(smoothed_values_1, color="green", alpha=0.6, linewidth=2)
    # plt.plot(_LOSS, color="red", alpha=0.6, linewidth=2)
